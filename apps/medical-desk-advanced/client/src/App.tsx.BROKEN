          ...
          {activeTab === 'automacao' && (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                    Cadeias de Cuidado Clínico
                  </h2>
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-1`}>
                    Protocolos Bundle com timing crítico para melhores desfechos
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Zap className="w-6 h-6 text-yellow-500" />
                  <span className={`text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    {cadeiasCompletas.filter(c => c.indicadores && c.indicadores.pacientesAtivos > 0).length} cadeias ativas
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  <p className="text-4xl font-bold text-blue-600">
                    {cadeiasCompletas.filter(c => c.indicadores && c.indicadores.pacientesAtivos > 0).length}
                  </p>
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>
                    de {cadeiasCompletas.length} disponíveis
                  </p>
                </div>
                ...
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  <p className="text-4xl font-bold text-teal-600">
                    {cadeiasCompletas.reduce((sum, c) => sum + (c.indicadores?.pacientesAtivos || 0), 0)}
                  </p>
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>
                    em tratamento
                  </p>
                ...
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  <p className="text-4xl font-bold text-red-600">
                    {cadeiasCompletas.filter(c => c.prioridade === 'emergencia' && c.indicadores && c.indicadores.pacientesAtivos > 0).length}
                  </p>
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>
                ...
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  <p className="text-4xl font-bold text-green-600">
                    {Math.round(cadeiasCompletas.reduce((sum, c) => sum + (c.indicadores?.taxa_sucesso || 0), 0) / cadeiasCompletas.length)}%
                  </p>
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>
                    média geral
                  </p>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {cadeiasCompletas.map((cadeia, index) => {
                    const prioridadeColors = {
                      'emergencia': 'red',
                      'timing-critico': 'orange',
                      'urgente': 'yellow',
                      'eletivo': 'blue'
                    };
                    const color = prioridadeColors[cadeia.prioridade as keyof typeof prioridadeColors];
                    const hasActive = cadeia.indicadores && cadeia.indicadores.pacientesAtivos > 0;
                    ...
                    return (
                      <motion.div
                        ...
                        onClick={() => setCadeiaSelecionada(cadeia)}
                        className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6 cursor-pointer border-2 ${
                          hasActive ? `border-${color}-500` : 'border-transparent'
                        } hover:border-teal-500 transition-all relative overflow-hidden`}
                      >
                        {/* Badge de Prioridade */}
                        <div className={`absolute top-0 right-0 px-3 py-1 bg-${color}-600 text-white text-xs font-bold rounded-bl-lg uppercase`}>
                          {cadeia.prioridade === 'emergencia' && 'EMERGÊNCIA'}
                          {cadeia.prioridade === 'timing-critico' && 'TIMING CRÍTICO'}
                          {cadeia.prioridade === 'urgente' && 'URGENTE'}
                          {cadeia.prioridade === 'eletivo' && 'ELETIVO'}
                        </div>
                        ...
                        <div className="mt-8">
                          ...
                          {cadeia.indicadores && (
                            <div className="space-y-2 mb-4">
                              <div className="flex items-center justify-between text-sm">
                                <span className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Pacientes ativos</span>
                                <span className={`font-bold ${hasActive ? `text-${color}-600` : darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                                  {cadeia.indicadores.pacientesAtivos}
                                </span>
                              </div>
                              <div className="flex items-center justify-between text-sm">
                                <span className={`${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Tempo médio</span>
                                <span className={`font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                                  {cadeia.indicadores.tempoMedio}
                                </span>
                              </div>
                            </div>
                          )}
                          ...
                          {/* Janela Ótima */}
                          {cadeia.janelaOtima && (
                            <div className={`flex items-center space-x-2 p-2 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} mb-3`}>
                          ...
                          <button className="w-full py-2 text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-900/20 rounded-lg font-medium transition-colors text-sm">
                            Ver Detalhes →
                          </button>
                        </div>
                      </motion.div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {metricas.map((metrica, index) => {
                  ...
                  const Icon = icons[metrica.tipo];
                  const color = colors[metrica.tipo];
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {acessosRapidos.map((acesso, index) => {
                    ...
                    const Icon = icons[acesso.icon as keyof typeof icons] || AlertCircle;
                    const colors = {
                      triagem: 'red',
                      abcde: 'blue',
                      drogas: 'purple',
                    ...
                    const color = colors[acesso.categoria];
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => setAcessoSelecionado(acesso)}
                        className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6 text-left hover:shadow-xl transition-all border-2 border-transparent hover:border-${color}-500`}
                      >
              ...
              <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mb-4`}>
                  Distribuição por Prioridade (Hoje)
                </h3>
                <div className="space-y-3">
                  {distribuicaoPrioridade.map((item, index) => {
                    const corClasses = {
                      'Vermelho': 'bg-red-600',
                    ...
                    return (
                      <div key={index} className="flex items-center space-x-4">
                        <div className={`w-3 h-3 ${corClasses[item.nivel as keyof typeof corClasses]} rounded-full`} />
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                          ...
                          <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                            <motion.div
                              ...
                              className={`h-full ${corClasses[item.nivel as keyof typeof corClasses]}`}
                            />
                          </div>
                        </div>
                      </div>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} mt-1`}>
                    Inteligência epidemiológica para decisões baseadas em evidências
                  </p>
                </div>
                <select
                  value={regiaoSelecionada}
                  onChange={(e) => setRegiaoSelecionada(Number(e.target.value))}
                  className={`px-4 py-2 rounded-lg border ${
                    darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'
                  }`}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                {Object.entries(dadosBrasil[regiaoSelecionada].indicadores).map(([key, value], index) => {
                  const labels: any = {
                    diabetes: 'Diabetes',
                  ...
                  const icons = {
                    diabetes: Heart,
                    hipertensao: TrendingUp,
                    obesidade: Users,
                    tabagismo: AlertTriangle,
                    alcoolismo: AlertTriangle
                  };
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      transition={{ delay: index * 0.1 }}
                      className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-4`}
                    >
              ...
              {/* Info da Região */}
              <div className={`${darkMode ? 'bg-gradient-to-r from-teal-900/20 to-blue-900/20' : 'bg-gradient-to-r from-teal-50 to-blue-50'} rounded-xl p-6`}>
                <div className="flex items-center space-x-4">
                  <MapPin className="w-12 h-12 text-teal-600" />
                  <div>
                    <h3 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      {dadosBrasil[regiaoSelecionada].estado || dadosBrasil[regiaoSelecionada].regiao}
                    </h3>
                    <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      População: {dadosBrasil[regiaoSelecionada].populacao}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mb-4`}>
                  Prevalências por Condição
                </h3>
                <div className="space-y-3">
                  {dadosBrasil[regiaoSelecionada].prevalencias.map((prev, index) => {
                    const TendenciaIcon = prev.tendencia === 'subindo' ? TrendingUp : 
                                          prev.tendencia === 'descendo' ? TrendingDown : 
                                          Activity;
                    const tendenciaColor = prev.tendencia === 'subindo' ? 'text-red-600' : 
                                          prev.tendencia === 'descendo' ? 'text-green-600' : 
                                          'text-gray-600';
                    ...
                    return (
                      <motion.div
                        ...
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: index * 0.1 }}
                        className={`flex items-center space-x-4 p-3 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}
                      >
              ...
              <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mb-4`}>
                  Tendências Sazonais - Condições Críticas (%)
                </h3>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={tendenciasSazonais}>
                    <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#374151' : '#e5e7eb'} />
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="respiratorias" stroke="#3b82f6" strokeWidth={2} name="Respiratórias" />
                    <Line type="monotone" dataKey="cardiovasculares" stroke="#ef4444" strokeWidth={2} name="Cardiovasculares" />
                    <Line type="monotone" dataKey="infecciosas" stroke="#10b981" strokeWidth={2} name="Infecciosas" />
                    <Line type="monotone" dataKey="trauma" stroke="#f59e0b" strokeWidth={2} name="Trauma" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  <div className="space-y-3">
                    {causasMortalidade.map((causa, index) => (
                      <div key={index} className="flex items-center justify-between">
                        <div className="flex items-center space-x-3 flex-1">
                          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: causa.cor }} />
                          <div className="flex-1">
                            <p className={`text-sm font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                              {causa.causa}
                            </p>
                            <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                              {causa.mortes_ano} mortes/ano
                            </p>
                          </div>
                        </div>
                        <span className={`text-lg font-bold ${darkMode ? 'text-teal-400' : 'text-teal-600'}`}>
                          {causa.percentual}%
                        </span>
                      </div>
                    ))}
                ...
                <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        data={causasMortalidade}
                        cx="50%"
                        cy="50%"
                        labelLine={false}
                        label={({ causa, percentual }) => `${causa.split(' ')[0]} ${percentual}%`}
                        outerRadius={100}
                        fill="#8884d8"
                        dataKey="percentual"
                      >
                        {causasMortalidade.map((entry, index) => (
              ...
              <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mb-4 flex items-center space-x-2`}>
                  ...
                  <span>Alertas Epidemiológicos Ativos</span>
                </h3>
                <div className="space-y-3">
                  {alertasEpidemiologicos.map((alerta, index) => {
                    const severidadeColors = {
                      alta: 'border-red-500 bg-red-50 dark:bg-red-900/10',
                      media: 'border-orange-500 bg-orange-50 dark:bg-orange-900/10',
                      baixa: 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/10'
                    };
                    return (
                      <motion.div
                        key={alerta.id}
                        ...
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: index * 0.1 }}
                        className={`p-4 rounded-lg border-2 ${severidadeColors[alerta.severidade]}`}
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <h4 className={`font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mb-1`}>
                              {alerta.titulo}
                            </h4>
                          ...
                          <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${
                            alerta.severidade === 'alta' ? 'bg-red-600 text-white' :
                            alerta.severidade === 'media' ? 'bg-orange-600 text-white' :
