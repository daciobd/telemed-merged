...
-   **Documentação**:
    ...
    -   `apps/telemed-deploy-ready/public/tour.html`: **[APRIMORADO - V2.0 QA EDITION + STUBS]** Guia estático completo (56KB) com 27 cards, toggle Paciente/Médico, badges coloridos, navegação por papel e 42 links mapeados para rotas reais. **Novas features de QA**: `<base href="/">` para links relativos, **verificador automático de links** (intercepta cliques em target="_blank", faz HEAD request, alerta se página não existe com sugestão de arquivo), **auditoria automática na carga** (console.table com status 200/404 de todos os links internos), normalização de URLs (trailing slash → index.html), e **comentários TODO** com lista de arquivos necessários no build. Previne "cair" silenciosamente na Landing quando link não existe. **TODOS OS LINKS FUNCIONAIS** - stubs criados para: `/auth/register.html`, `/patient/waiting-room.html`, `/patient/phr.html`, `/medicaldesk-demo/index.html`, `/medicaldesk-demo/agenda.html` (nota: MedicalDesk em `/medicaldesk-demo/` para evitar conflito com proxy `/medicaldesk/*` que roteia para servidor externo)
