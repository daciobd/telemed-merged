...
-   **Documentação**:
    ...
    -   `apps/telemed-deploy-ready/public/GUIA-TESTE-MEDICO.md`: **[NOVO - V2.0]** Guia completo de testes para médicos - documentação estruturada com 5 seções principais (Login, Dashboard, Widget, Consulta Jitsi, Páginas Complementares). Cada seção inclui: URL canônica, objetivo da página, checklist visual (👀 O que observar), análise técnica (🧠 O que entender), fluxo de navegação (➡️ Para onde seguir), e template de feedback estruturado (💬 Onde deixar impressões). Inclui credenciais mockadas, diagrama de fluxo completo, bugs possíveis, tempo estimado por etapa, e template de relatório final com avaliações por estrelas. **Essencial para QA e testadores** que precisam validar o fluxo médico end-to-end (Login → Dashboard → Widget → Consulta Jitsi).
    -   `apps/telemed-deploy-ready/public/guia-teste-medico.html`: **[NOVO - V2.0]** Versão HTML interativa do guia de testes - interface visual moderna com progress tracker (5 etapas), fluxo visual animado, credential box com dados de teste copy-to-clipboard, checklists interativos (localStorage), info cards coloridos, alerts contextuais (✅ sucesso / ⚠️ atenção / ❌ bugs), rating box com estrelas clicáveis, e CTAs para testar direto (links para /auth/login.html, /dashboard/, /consulta/). Design responsivo com gradientes roxos, seções expansíveis, e footer com link para versão Markdown. **Primeiro ponto de entrada recomendado** para novos testadores da jornada do médico.
-   **Botão MedicalDesk Atualizado**: **[COMPLETO - V3.0]** Botão "🔍 Buscar Protocolo" na página /consulta/ implementa busca interativa de protocolos clínicos via API local integrada (`/api/protocols/:condition` e `/api/mda/protocols/:condition`). **Interface completa**: prompt de busca por condição (hipertensão, diabetes, IAM, asma, pneumonia), loading visual com ícone animado, modal de resultados com seções estruturadas (📝 Descrição, 🔍 Diagnóstico com critérios/exames, 💊 Tratamento com lifestyle/medicações em cards coloridos, 📅 Acompanhamento com frequência/monitoramento), e tratamento de erros elegante com modal amarelo. **API 100% funcional**: 5 protocolos clínicos completos (hipertensão, diabetes, iam, asma, pneumonia) servidos localmente via `apps/telemed-internal/src/index.js` (linhas 583-677). **Auth bypass configurado**: rotas públicas sem autenticação JWT (linhas 551-555). Substitui botão antigo que tentava abrir /medical-desk-advanced/index.html (404).
