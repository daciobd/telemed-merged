...
var server = app.listen(PORT, "0.0.0.0", () => {
  console.log(`\u{1FA7A} TeleMed running on port ${PORT}`);
});
...
app.post("/api/auth/login", (req, res) => {
  ...
  const token = Buffer.from(JSON.stringify({ sub: id, role, exp: Date.now() + 864e5 })).toString("base64");
...
var staticPath = path.join(process.cwd(), "apps", "telemed-deploy-ready");
