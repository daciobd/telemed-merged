...
import * as aiHandlers from './routes/ai.js';
import * as demoHandlers from './routes/demo.js';
import * as metricsHandlers from './routes/metrics.js';
import { installTelemetry, handleTelemetryEvent, handleTelemetryMetrics, handleTelemetryPing } from './server/telemetry.js';

// Define __dirname para ES6 modules
...
const server = http.createServer((req, res) => {
  ...
  // ===== Atalhos curtos para apresentações =====
  // Dr. AI (auto-play)
  if (req.method === 'GET' && pathname === '/apresentacao') {
    res.writeHead(302, { 'Location': '/public/dr-ai-demo.html?autoplay=1' });
    res.end();
    return;
  }

  // Toasts (auto-play)
  if (req.method === 'GET' && pathname === '/apresentacao-toasts') {
    res.writeHead(302, { 'Location': '/public/toast-demo.html?autoplay=1' });
    res.end();
    return;
  }

  // Página seletora com links para ambas
  if (req.method === 'GET' && pathname === '/apresentacoes') {
    const presentationsPath = path.join(__dirname, 'public', 'presentations.html');
    fs.readFile(presentationsPath, (err, content) => {
      if (err) {
        res.writeHead(404);
  ...
  // Master demo que orquestra sequências
  if (req.method === 'GET' && pathname === '/apresentacao-master') {
    const masterPath = path.join(__dirname, 'public', 'master-demo.html');
    fs.readFile(masterPath, (err, content) => {
      if (err) {
        res.writeHead(404);
        res.end('404 Not Found');
        return;
  ...
  if (req.method === 'POST' && pathname === '/api/telemetry/event') {
    handleTelemetryEvent(req, res);
    return;
  }

  if (req.method === 'GET' && pathname === '/api/telemetry/metrics') {
    handleTelemetryMetrics(req, res);
    return;
  }

  if (req.method === 'GET' && pathname === '/api/telemetry/ping') {
    handleTelemetryPing(req, res);
    return;
  }

  // === ROTAS DE LOGS (audit logger) ===
  if (req.method === 'OPTIONS' && pathname === '/api/logs') {
    res.writeHead(200, {
  ...
  if ((urlPath === '/phr' || urlPath.startsWith('/phr/') || urlPath === '/registro-saude') && urlPath !== '/phr.html') {
    fs.readFile(path.join(__dirname, 'phr-react.html'), (err, content) => {
      if (err) {
        res.writeHead(404);
  ...
  // Resolve the file path within the server directory (not cwd)
...
// Instalar telemetria anônima
installTelemetry().catch(err => {
  console.error('Failed to install telemetry:', err);
});
